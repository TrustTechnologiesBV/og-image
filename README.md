...
# [Trust Open Graph Image as a Service](https://og-image.now.sh)
...
Now try changing the text to the title of your choosing and watch the magic happen ✨

## Getting Started

For local development it might be a bit tricky to get to work, because of the headless chrome instance.

1. `npm install`
2. Run locally with `vercel dev` and visit [localhost:3000](http://localhost:3000) (if nothing happens, run `npm install -g vercel`)
3. Visit `localhost:3000` to see what's happening.

### Technologies.

- [Dot Dom](https://github.com/wavesoft/dot-dom): A tiny (512 byte) virtual DOM template engine for embedded projects
- [marked](https://github.com/markedjs/marked): Low-level compiler for parsing **markdown** without caching or blocking for long periods of time.

## Notes

- Change the 'VERSION' variable if you're doing any major changes.
